...
void SilentTripleGen::init(u64 n, u64 batchSize, u64 numThreads, Mode mode,
  ...
  if (mode & Mode::Receiver) {
    ...
    mRecverOT =
        span<oc::SilentOtExtReceiver>(mBackingRecverOT.get(), numBatchs);
  }
...
void SilentTripleGen::setBaseOts(span<block> recvOts,
  ...
  for (u64 i = 0; i < mSenderOT.size(); i++) {
    mSenderOT[i].setSilentBaseOts(
        sendOts.subspan(0, mSenderOT[i].silentBaseOtCount()));
    sendOts = sendOts.subspan(mSenderOT[i].silentBaseOtCount());
  }

  for (u64 i = 0; i < mRecverOT.size(); i++) {
    mRecverOT[i].setSilentBaseOts(
        recvOts.subspan(0, mRecverOT[i].silentBaseOtCount()));
...
Proto SilentTripleGen::expand(coproto::Socket& chl) {
  ...
  if (mSenderOT.size()) {
    ...
    for (j = 0; j < mRecverOT.size(); ++j) {
      co_await(mRecverOT[j].silentReceive(recvOtChoiceBits, recvMsg, mPrng,
                                          chl));
